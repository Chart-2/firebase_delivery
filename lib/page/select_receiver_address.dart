...
class ReceiverPickResult {
  ...
  final String addressId;        // delivery.addressid_receiver (doc id ของ user_address)
...
class SelectReceiverAddressPage extends StatefulWidget {
  ...
  @override
  State<SelectReceiverAddressPage> createState() => _SelectReceiverAddressPageState();
}
...
class _SelectReceiverAddressPageState extends State<SelectReceiverAddressPage> {
  ...
  Future<void> _searchByPhone() async {
    ...
    final input = USE_NORMALIZED ? _normalizePhone(_phoneCtrl.text) : _phoneCtrl.text.trim();
  ...
  void _pickAddr(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    if (_foundUserId == null || _foundPhone == null || _foundName == null) return;
            ...
            child: Container(
              decoration: const BoxDecoration(
                gradient: LinearGradient(
                  colors: [Colors.black87, Colors.black54, brandRed, Colors.black],
                      ...
                      const Expanded(
                        child: Text(
                          ...
                          style: TextStyle(
                            ...
                            color: Colors.black,
                            shadows: [Shadow(blurRadius: 1.2, offset: Offset(0.8, 0.8), color: Colors.white)],
                          ),
                        ),
                      ),
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              validator: (v) {
                                final t = USE_NORMALIZED ? _normalizePhone(v ?? '') : (v ?? '').trim();
                                if (t.isEmpty) return 'กรุณาป้อนเบอร์ผู้รับก่อน';
                                if (t.length < 9) return 'รูปแบบเบอร์ไม่ถูกต้อง';
                                return null;
                              },
                            ),
                          ),
                              ...
                              return InkWell(
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      const CircleAvatar(radius: 18, child: Icon(Icons.home)),
                                          ...
                                          children: [
                                            RichText(
                                              text: TextSpan(
                                                ...
                                                children: [
                                                  const TextSpan(text: 'ชื่อ ', style: TextStyle(fontWeight: FontWeight.w900)),
                                                  TextSpan(text: _foundName ?? '-'),
                                                  const TextSpan(text: '  |  '),
                                                  const TextSpan(text: 'เบอร์ ', style: TextStyle(fontWeight: FontWeight.w900)),
                                                  TextSpan(text: _foundPhone ?? '-'),
                                                ],
                                              ),
                                            ),
